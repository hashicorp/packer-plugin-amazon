...
builder](builders/amazon-ebs), which is much easier to use.
...
// There are other ways to [set variables](templates/hcl_templates/variables#assigning-values-to-build-variables), including from a var
...
This builder generates data that are shared with provisioner and post-processor via build function of [template engine](templates/legacy_json_templates/engine) for JSON and [contextual variables](templates/hcl_templates/contextual-variables) for HCL2.
