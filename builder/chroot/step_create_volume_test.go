...
func buildTestRootDevice() *ec2types.BlockDeviceMapping {
...
func TestCreateVolume_io1_to_io1(t *testing.T) {
	...
	testRootDevice := buildTestRootDevice()
	testRootDevice.Ebs.VolumeType = ec2types.VolumeTypeIo1
	testRootDevice.Ebs.Iops = aws.Int32(1000)
	ret, err := stepCreateVolume.buildCreateVolumeInput("test-az", testRootDevice)
	assert.NoError(t, err)
...
func TestCreateVolume_gp2_to_io1(t *testing.T) {
	stepCreateVolume := StepCreateVolume{RootVolumeType: ec2types.VolumeTypeIo1}
	testRootDevice := buildTestRootDevice()
