...
- `root_volume_type` (ec2types.VolumeType) - The type of EBS volume for the chroot environment and resulting AMI. The
...
- `ami_architecture` (ec2types.ArchitectureValues) - what architecture to use when registering the final AMI; valid options
  are "arm64", "arm64_mac", "i386", "x86_64", or "x86_64_mac". Defaults to "x86_64".

- `boot_mode` (ec2types.BootModeValues) - The boot mode. Valid options are `legacy-bios` and `uefi`. See the documentation on
